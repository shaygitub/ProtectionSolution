...
int VerfifyDepDirs(WCHAR* AppDataPath) {
    ...
    WCHAR DeleteCommand[MAX_PATH * 2] = L"rmdir /s /q ";
    wcscat_s(DeleteCommand, AppDataPath);
    wcscat_s(DeleteCommand, L"\\ProtectionSolution");
    if (AppDataPath == NULL || _wsystem(DeleteCommand) == -1) {
