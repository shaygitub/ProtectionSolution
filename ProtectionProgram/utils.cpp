...
int VerfifyDepFiles(const char* FileHostIp, WCHAR* AppDataPath) {
    ...
    const char* FileCommands[] = { "cd `\\ProtectionSolution\\Service\\ && ",
        ...
        "if not exist ProtectionDriverChecker.exe curl http://~:45454/ProtectionDriverChecker/x64/Release/ProtectionDriverChecker.exe --output ProtectionDriverChecker.exe" };
